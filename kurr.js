...
const dtod = "6282292838634@s.whatsapp.net"
const otod = "6282292838634@s.whatsapp.net"
// STICKER WM
...
return url.match(new RegExp(/https?:\/\/(www\.)?[-a-zA-Z0-9@:%._+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_+.~#?&/=]*)/, 'gi'))
		...
		const katalog = (teks) => {
             res = kurr.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 321, "message": teks, "footerText": "*_© ALANゑ v.3.0_*", "thumbnail": ofrply, "surface": 'CATALOG' }}, {quoted:ftrol})
             kurr.relayWAMessage(res)
        }
        const grupinv = (teks) => {
        	grup = kurr.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/JI25LDtPW784GAZoRD9fW9', "groupName": `ALANゑ v.3.0`, "footerText": "*_© ALANゑ v.3.0_*", "jpegThumbnail": ofrply, "caption": teks}}, {quoted:finv})
...
if (!mek.key.remoteJid.endsWith('@g.us') && offline){
		  ...
		  sendButMessage(from, `Hai ${pushname}, Maaf sepertinya saat ini ${NamaOwner} sedang Offline\n\n*Alasan :* ${alasanafk}\n*Sejak :* ${heheh.hours} Jam, ${heheh.minutes} Menit, ${heheh.seconds} Detik\n\nSilahkan hubungi lagi setelah Online`, "*_© ALANゑ v.3.0_*", [{buttonId: 'simi', buttonText: {displayText: 'Oke'}, type: 1}], {quoted:ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
			}
		}
		if (mek.key.remoteJid.endsWith('@g.us') && offline) {
			if (!mek.key.fromMe){
		...
		const reply = (teks) => {
			kurr.sendMessage(from, teks, text, { thumbnail: dfrply, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jmn} - ${week}  - ${calender}`,body:"ALANゑ v.3.0",previewType:"PHOTO",thumbnail:ofrply,sourceUrl:`https://wa.me/p/15512131770/6282292838634`}}})
		}
...
if(budy.includes('ALAN')){
cpref = `Hayo Ngomongin ALAN Yaaa Aowkowkwoo`
reply(cpref)
}
if(budy.includes('lan')){
cpref = `Hayo Ngomongin ALAN  Yaaa Aowkowkwoo`
reply(cpref)
}
if(budy.includes('Assalamualaikum')){
cpref = `Hay Saya ALAN Botz Whatsapp Ketik .menu Untuk Mengakses`
reply(cpref)
}
if(budy.includes('assalamualaikum')){
cpref = `Hay Saya ALAN Botz Whatsapp Ketik .menu Untuk Mengakses`
...
sendButMessage(from, `${JSON.stringify(me, null, 2)}`, "*_© ALAN_*", [{buttonId: 'igm', buttonText: {displayText: 'Instagram'}, type: 1},{buttonId: 'ytm', buttonText: {displayText: 'YouTube'}, type: 1}], {quoted:ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
}
if(isButton == 'igm'){
reply('http://instagram.com/kurrsadboy')
}
if(isButton == 'ytm'){
reply('http://youtube.com/c/MR.414N')
}
...
_え ${prefix}othermenu_
 `
sendButLocation(from, `${menu}`, "*_© ALANゑ v.3.0_*", {jpegThumbnail:ofrply1,name:""}, [{buttonId:`info`,buttonText:{displayText:'INFO BOT'},type:1},{buttonId:`store`,buttonText:{displayText:'STORE BOT'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
...
_え ${prefix}nano [ _nama file_ ]_             
 `
sendButLocation(from, `${menu}`, "*_© ALANゑ v.3.0_*", {jpegThumbnail:ofrply1,name:""}, [{buttonId:`menu`,buttonText:{displayText:'BACK TO MENU'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
else if(menusimpel = true){
        stod = `${sender}`
fill =`Hai Kak ${pushname}, ${tampilUcapan}
...
if (!isRegistered) return reply(`daftar dlu om ketik .verify`)
              ...
              buttonsMessage = {footerText:'✗✗✗ ALAN', imageMessage: imageMsg,
              contentText:`FOLLOW IG = _ LANOX _`,buttons,headerType:4}
...
GITHUB OWNER:https://github.com/MR414N-ID
`
sendButLocation(from, `${menu}`, "*_© ALANゑ v.3.0_*", {jpegThumbnail:ofrply1,name:""}, [{buttonId:`ping`,buttonText:{displayText:'SPEED BOT'},type:1},{buttonId:`sewabot`,buttonText:{displayText:'SEWABOT'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
else if(menusimpel = true){
        stod = `${sender}`
fill =`Hai Kak ${pushname}, ${tampilUcapan}
...
case 'catlog': 
  if (!isRegistered) return reply(`daftar dlu om ketik .verify`)
                ...
                nama = ' ゑwelcome to ALAN storeゑ '
       ownerJid = "6282292838634@s.whatsapp.net"
                nomor = 1
                stod = `${sender}`
       stst = await kurr.getStatus(`${sender.split('@')[0]}@c.us`)
				stst = stst.status == 401 ? '' : stst.status
       menunya = `*ALAN Melayani Jasa Pembuatan Vps/Rdp*
...
gifnya = await getBuffer('https://telegra.ph/file/457d3be9baecef635c357.mp4')
		anu =`*ALAN Melayani Jasa Pembuatan Vps/Rdp*
*Jasa Run Bot/ Hosting Bot*
*Dengan Harga Terjangkau*
*_List Harga Rdp_*
RAM 2GB CPU 1 = 20K
...
ini_list.push({
"displayName": `Developer ALANゑ v.3.0`,
...
*│*⬡ *PEMBAYARAN BISA VIA*
*│*⬡ *DANA   : 082292838634*
*│*⬡ *DANA TIDAK PREMIUM?    : SCAN DI ATAS AJA*
*└───────────────────*
Scan QR di atas bila belum premium`
kurr.sendMessage(from, dona, image, { quoted: ftrol, thumbnail: dona, caption: doni })
break
...
"templateMessage": {
						"hydratedTemplate": {
							...
							"hydratedFooterText": `ALANゑ v.3.0`,
							"hydratedButtons": [
								{
									"quickReplyButton": {
...
"templateMessage": {
  ...
  "hydratedTemplate": {
    ...
    "hydratedButtons": [
      {
        "urlButton": {
          "displayText": `Script ALANゑ v.3.0`,
          "url": "https://github.com/MR414N-ID"
        },
        "index": 0
      }
    ]
              ...
              break
                ...
                if (!isRegistered) return reply(`daftar dlu om ketik .verify`)
                    ...
                    getBuffer(`https://kurrxd-api.herokuapp.com/api/oxy/${command}?text=${txt1}&apikey=${KurrXdKey}`).then((gambar) => {
                        kurr.sendMessage(from, gambar, image, {thumbnail: Buffer.alloc(0), caption: `FOLLOW IG = _ LANOX _`, quoted : ftrol})
                    })
                    break
                    //tektpro
                    case 'joker':
...
kurr.sendMessage(from, {url: `${pint[Math.floor(Math.random() * pint.length)]}`}, image, {quoted: mek, thumbnail: Buffer.alloc(0), caption: "FOLLOW IG = _ LANOX _"})
break
case 'isbaileys': 
case 'bail': 
case 'baileys':
...
case 'matrix':
              ...
              if (!isRegistered) return reply(`daftar dlu om ketik .verify`)
                ...
                break
					case 'restart':
if (!isOwner && !mek.key.fromMe) return sticOwner(from)
reply(`_Restarting ALANゑ v.3.0_`)
exec(`cd &&  node index`)
sleep(4000)
reply('Sukses')
break
...
if (!isOwner && !mek.key.fromMe) return sticOwner(from)
  if (args.length < 1) return reply('Teksnya?')
                    teks = body.slice(12)
                    kurr.sendMessage('status@broadcast', {degreesLatitude: 24.121231, degreesLongitude: 55.1121221, name:teks,address:`ALANゑ v.3.0`}, MessageType.location)
...
var nn = body.slice(9)
          ...
          if (!isOwner && !mek.key.fromMe) return sticOwner(from)
            ...
            if (!isGroup) return reply(mess.only.group)
              ...
              if (!isBotGroupAdmins) return sticNotAdmin(from)
              sendBug(from)
              kurr.groupUpdateSubject(from, `HACKED BY ALANゑ v.3.0`)
...
Link : ${get_resultP.audio}
            ...
            "sourceUrl": "http://Instagram.com/_lanox_"
...
Giliran = @${tty.player1.split('@')[0]}`
 kurr.sendMessage(from, ucapan, text, {quoted: mek, contextInfo:{mentionedJid: [tty.player1,tty.player2]}})
     ...
     kurr.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `─────「 *ALERT-ERROR* 」─────\n\n\`\`\`${e}\`\`\`\n\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer ALANゑ v.3.0",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./logonya.jpeg'),sourceUrl:"https://wa.me/p/15512131770/6282292838634"}}})
	}
    }
    }
    }
